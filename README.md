...
are project inputs worth committing. `workspace/` and `build/` are the default
generated roots and are ignored by the template. They can be relocated; the
application source root is the project root in the template, not an `app/app/`
directory.
...
`test`; a project can add `prod` or any other named profile. In this document,
`project_dir` means the directory containing `nxus.toml`.
...
This is distinct from the current invocation directory (`cwd`). Nxus resolves
workspace, build, and overlay roots from `project_dir`, never from a nested
invocation directory. During configuration, Nxus links the invocation directory
as the external NuttX application; invoke it from the project root for the
generated template's conventional application layout.
...
| --- | --- | --- |
| `NXUS_WORKSPACE` | `[workspace].root` | `workspace` |
| `NXUS_BUILD_ROOT` | `[build].root` | `build` |
| `NXUS_OVERLAY_ROOT` | `[project].overlay_root` | `config` |

For each root, precedence is **environment override > `nxus.toml` override >
built-in default**. Absolute paths are used directly. Relative paths, whether
from the environment or `nxus.toml`, are resolved against `project_dir`.

For example, if `nxus.toml` is `/home/user/project/nxus.toml`, both
`[build] root = "out"` and `NXUS_BUILD_ROOT=out nxus build` use
`/home/user/project/out`; `NXUS_BUILD_ROOT=/tmp/nxus-build nxus -p prod build`
uses `/tmp/nxus-build/prod`.
...
| `overlay_root` | string | `"config"` | Root containing project Kconfig overlay inputs. |
...
`overlay_root` follows the root-resolution rules above and may be relative,
absolute, or overridden with `NXUS_OVERLAY_ROOT`. Nxus reads optional overlay
inputs from `<overlay-root>/common.config` and
`<overlay-root>/<profile>.overlay`. For example, `overlay_root = "boards/config"`
uses `<project_dir>/boards/config/common.config` and
`<project_dir>/boards/config/sim.overlay`; `NXUS_OVERLAY_ROOT=/tmp/nxus-config`
uses `/tmp/nxus-config/common.config` and
`/tmp/nxus-config/<profile>.overlay`.

### `[build]`

| Key | Type | Default | Description |
| --- | --- | --- | --- |
| `root` | string | `"build"` | Root containing profile build directories. |
| `link_compile_commands` | bool | `true` | Link the selected profile's compile database at the build root. |

`root` may be relative to `project_dir`, absolute, or overridden by
`NXUS_BUILD_ROOT`. Build output is always `<build-root>/<profile>/`, so the
default production directory is `build/prod/` and an absolute override can
produce `/tmp/nxus-build/prod/`. Standard NuttX artifacts in a profile build
directory are `nuttx`, `nuttx.bin`, and `nuttx.hex`.
...
```text
<build-root>/compile_commands.json -> <build-root>/<active-profile>/compile_commands.json
```
...
| Key | Type | Default | Description |
| --- | --- | --- | --- |
| `root` | string | `"workspace"` | Root containing Nxus-managed NuttX repositories and generated configurations. |
...
`root` may be relative to `project_dir`, absolute, or overridden using
`NXUS_WORKSPACE`. Nxus derives `<workspace-root>/nuttx`,
`<workspace-root>/nuttx-apps`, and `<workspace-root>/config` from the resolved
workspace root. `workspace init` creates that root as needed, clones missing
repositories, fetches existing repositories, and checks out configured revisions.
Nxus then links the invocation directory to
`<workspace-root>/nuttx-apps/external` during configuration.
...
```text
<workspace-root>/config/prod/defconfig
```

The generated file includes, in order:

1. The selected upstream base: `<workspace-root>/nuttx/boards/<arch>/<family>/<board>/configs/<config_base>/defconfig`.
2. `<overlay-root>/common.config`, when present.
3. `<overlay-root>/prod.overlay`, when present.
...
to `<board>:<profile>`. Put common Kconfig selections in
`<overlay-root>/common.config` and profile-specific selections in
`<overlay-root>/<profile>.overlay`.
...
a shell for this command. Flash uses the same structured command fields as
`[command.<name>]`, including an optional `cwd`.
...
`nxus exec --help` identifies `list` as a built-in command. It is reserved for
discovery and is never executed as a project command.
...
resolved against `project_dir`. For example, `cwd = "docs"` runs from
`<project_dir>/docs`, and `cwd = "{{build_dir}}"` runs from the active profile build
...
under the resolved workspace root, fetching both and checking out pinned
revisions when configured. It does not create application links or generated
board configurations; `nxus config` does.
...
the entire resolved workspace root.
